...
class SVDLinearSuper(nn.Module):
    def __init__(self,ratio=[1.0],in_channel=1024,out_channel=4096,bias=True):
        ...
        self.fc = torch.nn.Linear(in_channel, out_channel, bias=bias)
    ...
    def forward(self,x,scaling_factor=None):
        sample_dim = int(round(self.sample_ratio * self.num_components))
        if sample_dim == self.num_components:
            x = self.fc(x)
        else:
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,layer=None,is_svd=None,SVD_Config=None,drop_svd=False):
        ...
        if ((not self.is_svd and not self.drop_svd) or self.is_svd == None):
            ...
            if ratio1==None:
                ...
                self.fc1 = SVDLinearSuper(ratio1,in_features, hidden_features)
            if ratio2==None:
                self.fc2 = nn.Linear(hidden_features, out_features)
            else:
                self.fc2 = SVDLinearSuper(ratio2,hidden_features, out_features)
        self.act = act_layer()
