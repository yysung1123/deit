...
class WSSVD_Linear(nn.Module):
    def __init__(self,ratio=[1.0],in_channel=1024,out_channel=4096,bias=True):
        super().__init__()
        self.num_components = min(in_channel,out_channel)
        
        self.ratio = ratio.copy()
        self.ratio.sort()
        assert self.ratio[-1] == 1.0
    ...
    def set_weight(self):
        for (param, num_components) in zip(self.VT, self.svd_num_components):
            param.data = self.S.weight.data[:num_components, :]
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,layer=None,is_svd=None,SVD_Config=None,drop_svd=False):
        ...
        if ((not self.is_svd and not self.drop_svd) or self.is_svd == None):
            ...
            if ratio1==None:
                ...
                self.fc1 = WSSVD_Linear(ratio1,in_features, hidden_features)
            if ratio2==None:
                self.fc2 = nn.Linear(hidden_features, out_features)
            else:
                self.fc2 = WSSVD_Linear(ratio2,hidden_features, out_features)
        self.act = act_layer()
...
class resmlp_models(nn.Module):
    ...
    def set_weight(self):
        for name, module in self.named_modules():
            if isinstance(module, WSSVD_Linear):
                module.set_weight()
