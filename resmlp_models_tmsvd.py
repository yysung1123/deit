...
class TMSVD_Linear(nn.Module):
    def __init__(self,ratio=[1.0],in_channel=1024,out_channel=4096,bias=True):
        ...
        for n in self.svd_num_components:
            param = nn.Parameter(torch.eye(n))
            if n == self.num_components:
                param.requires_grad = False
            self.register_parameter(f"tm_{n}", param)
            self.transform_matrices.append(param)
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,layer=None,is_svd=None,SVD_Config=None,drop_svd=False):
        ...
        if ((not self.is_svd and not self.drop_svd) or self.is_svd == None):
            ...
            if ratio1==None:
                ...
                self.fc1 = TMSVD_Linear(ratio1,in_features, hidden_features)
            if ratio2==None:
                self.fc2 = nn.Linear(hidden_features, out_features)
            else:
                self.fc2 = TMSVD_Linear(ratio2,hidden_features, out_features)
        self.act = act_layer()
